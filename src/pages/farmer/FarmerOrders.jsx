...
const stats = [
  { label: 'Total Orders', value: '185', icon: PackageSearch, color: 'bg-blue-600' },
  { label: 'Processing', value: '42', icon: Clock, color: 'bg-amber-500' },
  { label: 'Completed', value: '130', icon: CheckCircle2, color: 'bg-emerald-600' },
  { label: 'Cancelled', value: '13', icon: XCircle, color: 'bg-red-500' },
];

const statusConfig = {
  Selesai: { color: 'bg-emerald-100 text-emerald-700', icon: CheckCircle2, label: 'Completed' },
  Diproses: { color: 'bg-amber-100 text-amber-700', icon: Clock, label: 'Processing' },
  Dikirim: { color: 'bg-blue-100 text-blue-700', icon: Truck, label: 'Shipped' },
  Dibatalkan: { color: 'bg-red-100 text-red-700', icon: XCircle, label: 'Cancelled' },
};
...
const allOrders = [
  ...recentOrders,
  { id: 'INV-009', customer: 'CV Tani Sejahtera', product: 'Cassava', total: 'Rp 3,750,000', status: 'Diproses' },
  { id: 'INV-010', customer: 'Mr. Hasan', product: 'Fresh Tomatoes', total: 'Rp 960,000', status: 'Dikirim' },
  { id: 'INV-011', customer: 'Koperasi Makmur', product: 'Sweet Potato', total: 'Rp 1,440,000', status: 'Selesai' },
  { id: 'INV-012', customer: 'UD Berkah Tani', product: 'Water Spinach', total: 'Rp 280,000', status: 'Dibatalkan' },
];
...
const FarmerOrders = () => {
  ...
  const filtered = allOrders.filter((o) => {
    const statusMap = { 'Processing': 'Diproses', 'Shipped': 'Dikirim', 'Completed': 'Selesai', 'Cancelled': 'Dibatalkan' };
    const matchTab = activeTab === 'All' || o.status === statusMap[activeTab];
    const matchSearch =
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div>
        <h2 className="text-base font-bold text-slate-800">Order Management</h2>
        <p className="text-xs text-slate-500 mt-0.5">Track and manage all incoming orders</p>
      </div>
      ...
      <div className="bg-white rounded-[6px] border border-slate-100 shadow-sm">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                <th className="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Order No.</th>
                <th className="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Customer</th>
                <th className="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Product</th>
                <th className="text-right px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Total Price</th>
                <th className="text-center px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Status</th>
                <th className="text-center px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Action</th>
              </tr>
            </thead>
        ...
        <div className="px-4 py-3 border-t border-slate-100 flex items-center justify-between">
          <p className="text-xs text-slate-400">Showing {filtered.length} out of {allOrders.length} orders</p>
